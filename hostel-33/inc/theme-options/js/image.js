jQuery(document).ready(function ($) {
  ...
  removeItem();
  ht33ImgAutoSortTxt();
  ht33ImgAutoSortCheckbox();
});

function ht33ImgAutoSortTxt() {
  jQuery(".ht33-auto-sort-text").bind("keyup", function (e) {
    var eContainer = jQuery("#" + jQuery(this).data("container"));
    saveAttachmentIds(eContainer);
  });
}

function ht33ImgAutoSortCheckbox() {
  jQuery(".ht33-auto-sort-checkbox").click(function () {
    var eContainer = jQuery("#" + jQuery(this).data("container"));
    saveAttachmentIds(eContainer);
  });
}
...
function removeItem() {
  jQuery(".removeit").click(function () {
    ...
    if (count_li == 0) {
      jQuery(eContainer).html('<span class="ht33-no-photo">' + jQuery(eContainer).data("nophoto") + '</span>');
    }
  });
}
...
function imageMultipleInit() {
  ...
  jQuery('.ht33-image-type .btn-img-field').on('click', function (e) {
...
function createImgBlockHtml(attachment, eContainer, is_multiple) { //alert(888); console.log(attachment);
  ...
  if (hideattribute == '0') {
    ...
    var arr_title_html = '<div class="attribute-it"><input type="text" data-container="' + container_id + '" class="attr-title ht33-auto-sort-text" placeholder="' + jQuery(eContainer).data('title') + '" value=""></div>';

    var arr_description_html = '<div class="attribute-it"><textarea data-container="' + container_id + '" class="attr-description ht33-auto-sort-text"  placeholder="' + jQuery(eContainer).data('description') + '"></textarea></div>';

    var arr_link_html = '<div class="attribute-it"><input type="text" data-container="' + container_id + '" class="attr-link ht33-auto-sort-text" placeholder="' + jQuery(eContainer).data('link') + '" value="' + img_link + '"></div>';

    var arr_link_title_html = '<div class="attribute-it"><input type="text" data-container="' + container_id + '" class="attr-link_title ht33-auto-sort-text" placeholder="' + jQuery(eContainer).data('link_title') + '" value=""></div>';

    var arr_color_html = '<div class="attribute-it"><input type="text" data-container="' + container_id + '" class="attr-color ht33-auto-sort-text" placeholder="' + jQuery(eContainer).data('color') + '" value=""></div>';

    var arr_target_html = '<div class="attribute-it"><input data-container="' + container_id + '" type="checkbox" class="attr-target widefat ht33-checkbox ht33-auto-sort-checkbox" id="' + uni_id + '"><label for="' + uni_id + '" class="checkbox_label">' + jQuery(eContainer).data('target') + '</label></div>';

    var attr_list = '<div class="ht33-attribute-list">'
      + arr_title_html
      + arr_description_html
      + arr_link_html
      + arr_link_title_html
      + arr_color_html
      + arr_target_html
...
function saveAttachmentIds(eContainer) {
  ...
  jQuery(aItems).each(function (index, element) {
    ...
    var attr_description = jQuery(jQuery(this).find('.attribute-it .attr-description').get(0)).val();
    arr_attr['description'] = attr_description;
