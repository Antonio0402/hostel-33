...
class Hostel33WebsiteSettings
{
  private $page_title = 'Website Settings';
  private $menu_title = 'Website Settings';
  private $menu_slug = 'hostel-33-website-settings';
  private $position = 100;
  private $settings_fields = 'hostel-33-website-settings-section';
  ...
  public function createMenuThemeOptionPage()
  {
    ...
    add_menu_page(__($page_title, 'hostel-33'), __($menu_title, 'hostel-33'), $capability, $menu_slug, $function_callback, $icon_url, $position);
  }
    ...
    <form method="post" action="options.php" <?php if ($is_tab) { ?> class="has-tabs" <?php } ?>>
      ...
      if ($arr_section) {
        ...
        foreach ($arr_section as $section => $arr_fields) {
          ...
          echo '<section id="' . $section . '" class="section ' . $cls . ' ' . $s_cls . '">';
    ...
    if ($this->is_tab) {
      ...
      if ($arr_section) {
        ...
        foreach ($arr_section as $section => $arr_item) {
          $s_cls = '';
          if (!$i) {
            $s_cls = 'nav-tab-active';
          }
          $section_title = $arr_item['title'];
          echo '<a id="tab_' . $section . '" class="nav-tab ht33-tab-btn ' . $s_cls . '" href="#' . $section . '"><span class="dashicons dashicons-arrow-right-alt2"></span>' . $section_title . '</a>';
          $i++;
        }
        echo '</h2>';
      }
...
// Run----------
new Hostel33WebsiteSettings();

// Danh sách các hàm lấy option----------------------------------
// Có thể dùng các hàm bên dưới đây
//--------------------------------------------------------------

// Test: ht33_get_option('twitter_url');
function ht33_get_option($field_id)
{
...
// Hàm lấy Url Hình ảnh: ht33_get_full_src('key_img'); @return: https://wpshare247.com/abc.jpg.....
function ht33_get_full_src($field_id)
{
  $arr_single_data = ht33_get_image_data($field_id);
...
// If single image @return more url attribute, if multiple images @return images
function ht33_get_image_data($field_id)
{
  $arr_field = ht33_get_option($field_id);
  if (!$arr_field) return '';
  $type = $arr_field['type'];
 ...
 */
function ht33_get_slider_data($field_id)
{
  $arr_field = ht33_get_option($field_id);
  $type = $arr_field['type'];
  if ($type == 'slider') {
...
$args = array(  'size' => 'thumbnail',
				'id' => 'slider_id',
				'class' => array('slider' => 'cls1 cls2', 'item' => 'item1 item2', 'a' => 'link1 link2', 'img' => 'img1 img2' ),
				...
				'slider_tag' => 'ul',
				'item_tag' => 'div',
				'description_open' => '<span class="description">',
				'description_close' => '</span>',
				'description_last' => true,
				'description_hide' => true,
			)
*/
function ht33_the_slider_html($field_id, $args)
{
  $arr_slides = ht33_get_slider_data($field_id);
  if ($arr_slides) {
    ...
    $description_open = (isset($args['description_open'])) ? $args['description_open'] : '<span class="description">';
    $description_close = (isset($args['description_close'])) ? $args['description_close'] : '</span>';
    $description_last = (isset($args['description_last'])) ? true : false;
    $description_hide = (isset($args['description_hide'])) ? true : false;
  ...
  ?>
    ...
    foreach ($arr_slides as $slide) {
      ...
      $description = $description_open . $slide_description . $description_close;
      $target = $slide['target'];
    ?>
      <?php if ($before_item) echo $before_item; ?>
      <?php echo '<' . $item_tag; ?> class="slide-item <?php echo $item_class; ?>">
      <?php if ($before_a) echo $before_a; ?>
      <a <?php if ($a_class) { ?> class="<?php echo $a_class; ?>" <?php }
                                                                if ($link) { ?> href="<?php echo $link; ?>" <?php }
                                                                                                          if ($target) { ?>target="_blank" <?php } ?>>
        <?php
        if ($description && !$description_last && $description_hide === false) {
          echo $description;
        } ?>
        <?php if ($before_img) echo $before_img; ?>
        <img class="<?php echo $img_class; ?>" src="<?php echo $attachment_url; ?>" alt="<?php echo $slide_description; ?>" />
...
function ht33_the_content_field($field_id)
{
  echo apply_filters('the_content', ht33_get_option($field_id));
}
