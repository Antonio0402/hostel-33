...
function get_slider_control_group($items, $target_key = null)
{
  ob_start();
?>
  <div part="button-prev" class="button-prev" tabindex="0" role="button" aria-label="Previous slide" aria-controls="swiper-container">
  ...
  <div part="pagination" class="pagination pagination-bullets pagination-horizontal">
    ...
    foreach ($items as $key => $value) {
      if ($target_key) {
        if ($value[$target_key] !== ''): ?>
          <span class="pagination-bullet" tabindex="0" role="button" aria-label="Go to slide <?php echo ($key + 1) ?>" aria-controls="swiper-container" aria-selected="false"></span>
        <?php endif;
      } else { ?>
        <span class="pagination-bullet" tabindex="0" role="button" aria-label="Go to slide <?php echo ($key + 1) ?>" aria-controls="swiper-container" aria-selected="false"></span>
    <?php }
